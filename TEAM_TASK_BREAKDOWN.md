...
|------|------|-----------------|-----------------|
| **Khurshid Normurodov** | Initial Project Architect (Phase 1) | Initial system architecture, project planning, repository structure | Architecture Design, Initial Planning |
| **Farzaneh Barzegar** | Project Lead / Data Ingestion Engineer | Project coordination, Kafka pipeline, repository management, raw event ingestion | Project Coordination, Kafka, Producer, Data Generation |
| **Hontar Daniil** | Data Processing & ML Engineer | Spark Streaming, feature engineering, ML training, MLflow tracking | Spark Jobs, Silver/Gold Layers, ML Training, Feature Engineering |
| **Elif Sila Okutucu** | Analytics & DevOps Engineer | Airflow orchestration, PostgreSQL integration, Grafana monitoring, Docker infrastructure, validation evidence | Airflow, PostgreSQL, Grafana, Monitoring, Validation |

### Project Ownership Note

Khurshid Normurodov contributed during the initial planning and architecture phase of the project. Ongoing project coordination, implementation tracking and documentation management were subsequently handled by Farzaneh Barzegar throughout the development lifecycle.
...
**Deliverables:**
- [x] Create GitHub repository with proper naming convention
- [x] Set up branch protection rules (main, develop, feature branches)
- [x] Create `.gitignore` (Python, Docker, ML artifacts)
- [x] Add CODE_OF_CONDUCT.md
- [x] Add CONTRIBUTING.md with development guidelines
- [x] Set up GitHub Projects board (Kanban)
- [x] Add team members as collaborators with appropriate permissions
- [x] Create pull request template
- [x] Set up GitHub Issues templates (feature, bug, documentation)
...
**Deliverables:**
- [x] Create `scripts/init_postgres.sql` with:
...
- [x] Create sample queries for common analytics
- [x] Test connections and permissions
...
**Khurshid's Tasks:**
- [x] Acquire or generate synthetic fraud dataset (21 features, 100K+ records)
- [x] Perform exploratory data analysis (EDA):
  - Data distribution analysis
  - Class balance analysis (fraud rate)
  - Feature correlations
  - Missing values analysis
- [x] Create `data/synthetic_fraud_dataset.csv`
- [x] Document dataset in `documentation/05_datasets.md`:
...
- [x] Document any data preprocessing needed
- [x] Test data loading in producer
...
**Deliverables:**
- [x] Create `producer/transaction_generator.py`:
...
- [x] Create `producer/Dockerfile`
- [x] Create `producer/requirements.txt`:
  - kafka-python
  - pandas
  - numpy
- [x] Add fraud injection logic (optional enhancement):
  - Inject synthetic fraud patterns (5 types)
  - Control fraud rate via environment variable
- [x] Document producer configuration
- [x] Add logging and error handling
- [x] Add health check endpoint (if applicable)
- [x] Create unit tests for data validation
...
**Deliverables:**
- [x] Create Bronze layer in `spark_jobs/fraud_streaming_job.py`:
...
- [x] Create Delta table configuration
- [x] Add schema validation
- [x] Add error handling and dead-letter queue
- [x] Add checkpointing
- [x] Document Bronze layer design
...
**Deliverables:**
- [x] Create Silver layer in `spark_jobs/fraud_streaming_job.py`:
...
**Deliverables:**
- [x] In Silver layer, add ML scoring:
...
- [x] Handle model version mismatches
- [x] Add inference latency monitoring
- [x] Document ML score integration
...
**Deliverables:**
- [x] Create Gold layer in `spark_jobs/fraud_streaming_job.py`:
...
**Deliverables:**
- [x] In `ml/train_model.py`, add MLflow tracking:
...
- [x] Create evaluation report (JSON/CSV)
...
**Deliverables:**
- [x] Create Airflow home directory structure:
  - `dags/` - DAG definitions
  - `logs/` - DAG logs
  - `plugins/` - Custom operators/sensors (if needed)
- [x] Create `dags/requirements.txt`:
  - apache-airflow
  - apache-airflow-providers-postgres
  - apache-airflow-providers-apache-spark
  - psycopg2-binary
  - delta-spark (if using Delta Lake)
- [x] Create Airflow connections in `dags/init_airflow.py`:
  - PostgreSQL connection
  - Spark connection
  - MLflow connection
- [x] Create `dags/config.yaml` with DAG parameters:
  - Schedule intervals
  - Retries
  - Timeouts
  - Alert emails
- [x] Document Airflow setup and access
...
- Airflow DAG registration validated
- DAG discovery confirmed through Airflow scheduler
- Manual DAG execution successfully completed
- Evidence stored in:
...
**Deliverables:**
- [x] Create `dags/fraud_detection_daily_dag.py`:
...
- [x] Add SLAs: 30 min per task
- [x] Add retry logic: 2 retries on failure
- [x] Add monitoring: task duration, success/failure logging
- [x] Add data quality checks before retraining
...
**Deliverables:**
- [x] Create `dags/data_quality_monitoring_dag.py`:
...
- [x] Add alerting thresholds (configurable)
- [x] Create `scripts/dq_checks.sql` with check queries
- [x] Add data quality metrics to PostgreSQL `dq_checks` table
- [x] Document DQ thresholds and actions
...
**Deliverables:**
- [x] Create custom Airflow operators/sensors for PostgreSQL:
  - PostgreSQL write operator (batch insert metrics)
  - PostgreSQL read sensor (wait for data)
- [x] Create `scripts/postgres_queries.sql`:
  - Sample queries for fraud metrics
  - DQ summary queries
  - Model performance queries
- [x] Set up connection pooling if needed
- [x] Add logging for all writes
- [x] Document table write schemas
...
**Deliverables:**
- [x] Create `grafana/provisioning/datasources/datasource.yaml`:
  - PostgreSQL data source configuration
  - Connection settings
  - Test connection works
- [x] Create `grafana/provisioning/dashboards/dashboard.yaml`:
  - Dashboard provisioning configuration
- [x] Create `grafana/dashboards/fraud_alerts_dashboard.json`:
  - **Dashboard Panels**:
    1. **KPI Cards**:
...
- [x] Test all panels with sample data
...
- Fraud Alerts Monitoring Dashboard created
- Dashboard connected to PostgreSQL datasource
- Real-time metrics visualization verified
...
- [x] Create `tests/test_producer.py`:
...
- [x] Create `tests/test_spark_job.py`:
...
- [x] Create TEAM_ROLES.md (what each person owns)
...
**Elif - Evidence Updates:**
- [x] Added final Grafana dashboard export to `grafana/dashboards/`
- [x] Added Grafana monitoring documentation in `grafana/README.md`
- [x] Added PostgreSQL validation evidence screenshots
- [x] Added Airflow DAG registration and execution evidence screenshots
- [x] Updated evidence documentation under `documentation/evidence/`
- [x] Updated main README with dashboard validation summary

**All Team Members - Component Docs:**
- [x] Update each component's README:
  - `producer/README.md`
...
- **Tasks**: 6.3, 6.4
- **Deliverable**: Complete documentation, validation evidence, project portfolio readiness
...
- [x] Airflow orchestration validated

### Code Quality
- [x] Code coverage ≥ 80% (unit tests)
- [x] All components documented
- [x] No hardcoded values (use .env)
- [x] Consistent code style (linting passes)
- [x] Proper error handling throughout

### Operations
- [x] System runs 24/7 without manual intervention
- [x] All team members can deploy independently
...
- [x] Regular sync meetings (2x/week)
- [x] No blocking issues on main branch
- [x] Knowledge sharing documented (wikis, READMEs)
...
### Decision Making
- **Architecture**: Team decision based on the initial design and implementation constraints
...
### Phase 1 (Week 1)
- [x] GitHub repo created with all settings
- [x] Docker Compose running all services
- [x] PostgreSQL initialized
- [x] Dataset loaded and validated
- [x] All team members can clone and run `make up`

### Phase 2 (Week 2-3)
- [x] Kafka producer sending transactions
- [x] Bronze layer ingesting all data
- [x] Silver layer computing features
- [x] Gold layer flagging fraud
- [x] All outputs (Delta, Kafka, PostgreSQL) working

### Phase 3 (Week 3)
- [x] ML model training successfully
- [x] MLflow tracking experiments
- [x] Model evaluation working
- [x] Model integrated into streaming

### Phase 4 (Week 4)
- [x] Airflow daily retraining DAG running
- [x] Hourly DQ monitoring running
- [x] PostgreSQL receiving all writes
- [x] All SLAs met

### Phase 5 (Week 5)
- [x] Grafana dashboards live
- [x] All panels showing correct data
